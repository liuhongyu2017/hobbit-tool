...
import com.hobbit.core.tool.utils.ImageUtil;
...
public class BladeFileProxyFactory implements IFileProxy {
  ...
  public String[] path(File f, String dir) {
    ...
    return new String[]{BladeFileUtil.formatUrl(uploadPath.toString()),
        BladeFileUtil.formatUrl(virtualPath.toString())};
  }
  ...
  public static String getFileDir(String dir, String saveDir) {
    ...
    newFileDir.append(saveDir)
        .append(File.separator).append(dir).append(File.separator)
  ...
  public void compress(String path) {
    try {
      ImageUtil.zoomScale(ImageUtil.readImage(path), new FileOutputStream(new File(path)), null,
          SystemConstant.me().getCompressScale(), SystemConstant.me().isCompressFlag());
